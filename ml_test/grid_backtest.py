...
ATR_PERIOD         = 14     # ATR period on 15-min bars for dynamic spacing
ATR_MULTIPLIER     = 0.5    # grid spacing = ATR * this multiplier
MIN_SPACING        = 1.0    # floor (spread would eat anything smaller)
MAX_SPACING        = 15.0   # ceiling (too wide = too few TPs)
...
def load_regimes(path):
    ...
    return df[['regime', 'ranging_score_biased', 'session', 'High', 'Low', 'Close']]
...
print(f'Grid spacing: dynamic ({ATR_MULTIPLIER}x ATR14, min=${MIN_SPACING}, max=${MAX_SPACING})  |  Levels: {GRID_LEVELS} each side')
...
# Compute ATR on 15-min bars and align to 5-min (same shift(1) logic)
...
df15['atr']  = tr.ewm(span=ATR_PERIOD, adjust=False).mean()
atr_reindexed = df15['atr'].shift(1).reindex(combined_idx).ffill()
df5['atr15'] = atr_reindexed.reindex(df5.index)

df5 = df5.dropna(subset=['grid_center', 'atr15'])
...
for dt, bar in df5.iterrows():
    ...
    # Dynamic spacing: ATR-based so grid adapts to current volatility
    grid_spacing = float(np.clip(bar['atr15'] * ATR_MULTIPLIER, MIN_SPACING, MAX_SPACING))
