...
# =============================================================================
TARGET = 0.100        # Target profit in dollars (e.g., 5.0 = $5)
STOP = 0.050          # Stop loss in dollars (e.g., 2.5 = $2.5)
HORIZON = 26        # Number of bars to look ahead for outcome
...
print('='*80)
print('USDJPY 1MIN FEATURE CORRELATION ANALYSIS')
print('='*80)

print('\nLoading data...')
# New format: Date, Time, Open, High, Low, Close, TickVol, Vol, Spread
df = pd.read_csv('data/raw/USDJPY1.csv', sep='\t', 
...
with open('models/logistic_regression_usdjpy.pkl', 'wb') as f:
    pickle.dump(lr, f)
print('Saved: models/logistic_regression_usdjpy.pkl')

with open('models/random_forest_usdjpy.pkl', 'wb') as f:
    pickle.dump(rf, f)
print('Saved: models/random_forest_usdjpy.pkl')

with open('models/scaler_usdjpy.pkl', 'wb') as f:
    pickle.dump(scaler, f)
print('Saved: models/scaler_usdjpy.pkl')

# Save feature names for reference
with open('models/feature_names_usdjpy.txt', 'w') as f:
    f.write('\n'.join(all_features))
print('Saved: models/feature_names_usdjpy.txt')
...
test.to_csv('data/processed/USDJPY_feature_analysis.csv')
print(f'\nSaved test results: data/processed/USDJPY_feature_analysis.csv')
...
if len(hedge_df) > 0:
    ...
    hedge_df.to_csv('data/processed/HEDGE_strategy_usdjpy.csv', index=False)
    print(f'\nSaved: data/processed/HEDGE_strategy_usdjpy.csv')
