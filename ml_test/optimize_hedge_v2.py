...
if __name__ == '__main__':
    ...
    print(f'  {"#":>2s}  {"t":>6s}  {"Mode":>5s}  {"TGT":>4s}  {"SL":>4s}  '
          f'{"HOR":>3s}  {"TH":>4s}  {"Dep":>3s}  {"Tr":>3s}')
    print(f'  {"─"*45}')
    for rank, (_, row) in enumerate(trials_df.iterrows(), 1):
        md = str(row.get('params_mode', '?'))[:5]
        print(f'  {rank:2d}  {row["value"]:6.2f}  {md:>5s}  '
              f'{row["params_target"]:4.1f}  {row["params_stop"]:4.2f}  '
              f'{int(row["params_horizon"]):3d}  {row["params_rf_threshold"]:4.2f}  '
              f'{int(row["params_rf_depth"]):3d}  {int(row["params_rf_trees"]):3d}')
