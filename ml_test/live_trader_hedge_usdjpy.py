"""
USDJPY Live Hedge Trader using MetaTrader 5
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('live_trader_hedge_usdjpy.log'),
...
# MT5 Configuration
SYMBOL = 'USDJPYb'  # Adjust suffix based on your broker
...
TARGET_DOLLARS = 0.100  # $0.100 target (fixed)
STOP_DOLLARS = 0.050  # $0.050 stop (fixed)
MAGIC_NUMBER_LONG = 223456
MAGIC_NUMBER_SHORT = 754321

# Load trained model
try:
    with open('models/random_forest_usdjpy.pkl', 'rb') as f:
        model = pickle.load(f)
    with open('models/feature_names_usdjpy.txt', 'r') as f:
...
def main():
    parser = argparse.ArgumentParser(description='USDJPY Hedge Live Trader')
    ...
    logger.info("="*80)
    logger.info(f"USDJPY HEDGE TRADER - {mode_str}")
    logger.info("="*80)
