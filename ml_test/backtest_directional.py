...
DATA_PATH = os.path.join(os.path.dirname(__file__), '..', 'data', 'raw', 'XAUUSD2024.csv')
...
def run_backtest(df_test, rf, classes, threshold, target, stop):
    """
...
if __name__ == '__main__':
    ...
    report_feature_importance(rf)

    # ── Save outputs ─────────────────────────────────────────────────────
    os.makedirs(OUT_DIR, exist_ok=True)

    model_path = os.path.join(OUT_DIR, 'directional_rf_model.pkl')
    with open(model_path, 'wb') as f:
        pickle.dump({'model': rf, 'features': FEATURE_NAMES,
    ...
    trades_path = os.path.join(OUT_DIR, 'directional_trades.csv')
    ...
    print(f'  Best: TGT=${TARGET}  SL=${STOP}  HOR={HORIZON}  TH={RF_THRESHOLD}')
    print(f'  Total P&L: ${df_trades["pnl"].sum():+,.2f}  |  Trades: {len(df_trades):,}')
    print(f'{"="*72}\n')
